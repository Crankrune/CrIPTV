...
PLAYLIST_URL: str = "http://drewlive2423.duckdns.org:8081/DrewLive/MergedPlaylist.m3u8"


def get_clean_channels() -> list[Channel] | None:
    """Fetches the playlist, loads the JSON, and generates the base cleaned channels."""
    with open("data/drewlive_info.json", mode="r", encoding="utf-8") as f:
...
def make_clean_playlist(clean_channels: list[Channel]) -> None:
    ...
    with open(
        "output/playlists/playlist_DrewLive_clean.m3u", mode="w", encoding="utf-8"
    ) as fl:
...
def make_working_playlist(clean_channels: list[Channel]) -> None:
    ...
    with open(
        "output/playlists/playlist_DrewLive_working.m3u", mode="w", encoding="utf-8"
    ) as fl:
